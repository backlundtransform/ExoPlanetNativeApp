...
export const storeBase64 = async () => {
    ...
    if (value === null) {
        const colors = await fetch(
          `${apiurl}/ExoSolarSystems/GetImages`,
            {
                headers: {
...
export const GetPlanetListAsync = (
    ...
    const planetList = fetch(
        `${apiurl}/ExoSolarSystems/ExoPlanets?${filterstring}&%24top=${top}&%24skip=${skip}&%24orderby=${orderby}`,
    )
        .then(response => {
            return response.json()
        })
...
export const GetPlanetAsync = (name: string) => {
    const planetList = fetch(
        `${apiurl}/ExoSolarSystems/ExoPlanets?%24filter=Name eq '${encodeURIComponent(
            name,
        )}'`,
    )
        .then(response => {
...
export const GetHabitablePlanets = async () => {
    const planetList = await fetch(
        `${apiurl}/ExoSolarSystems/GetHabitablePlanets`,
    )
        .then(response => {
            return response.json()
        })
