...
export default class DrawPolyline extends React.PureComponent<
    { navigation: any },
    any
> {
    constructor(props) {
    ...
    render() {
        ...
        return (
            <React.Fragment>
                {lines.features.map((line, index) => (
                    <React.Fragment key={'fragment' + index}>
                        <Polyline
                            key={'line' + index}
                            coordinates={(line.geometry
                                .coordinates as number[][]).map(p => {
                                return {
                                    latitude: p[1] as number,
                                    longitude: p[0] as number,
                                }
                            })}
                            strokeColor="#add7ed"
                            strokeWidth={1}
                            zIndex={1000000}
                        />
                        {line.properties.constellation && (
                            <Marker
                                onPress={() =>
                                    this.navigateToConstellation(
                                        line.properties.constellationid,
                                    )
                                }
                                coordinate={{
                                    latitude: (line.geometry.coordinates[0][1] +
                                        1) as number,
                                    longitude: line.geometry
                        ...
                        {(line.geometry.coordinates as number[][]).map(
                            (p, i) => {
                                return (
                                    <Circle
                                        key={`${p[1]}${p[0]}${i}`}
